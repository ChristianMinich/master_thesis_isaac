"""Build a simple room scene with obstacles, a goal marker, and a Unitree Go2.

The room is a square enclosed by four fixed walls. Static box obstacles are
scattered inside, a green goal marker (visual-only cylinder) marks the
navigation target, and the Go2 robot is spawned near one corner.
...
def add_camera(stage):
    """Add a viewer camera looking down at the room."""
    camera = UsdGeom.Camera.Define(stage, "/World/ViewCamera")
    xform_api = UsdGeom.XformCommonAPI(camera.GetPrim())
    xform_api.SetTranslate(Gf.Vec3d(-8.0, -8.0, 6.0))
    xform_api.SetRotate(Gf.Vec3f(60.0, 0.0, -45.0))
    return camera
...
def add_obstacles(world: World):
    """Create simple static blocks inside the room. Structured for later randomization."""
...
def main():
    ...
    world.reset()
    print("[INFO] Room scene ready. Running simulation (close the window or Ctrl+C to stop).")
